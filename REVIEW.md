...
| # | 行动项 | 原因 |
|---|--------|------|
| 1 | 确认 `.openclaw` 数据是否存在，或将脚本路径迁移到 `.hermes` | 多个脚本依赖 `.openclaw` 路径 |
| 2 | 验证 `import_quota_db.py` 能成功导入数据到 DB | 所有功能依赖定额数据 |
| 3 | 统一端口：后端 8000，前端 proxy → 8000 | 两套前端都无法连通后端 |
| 4 | 删除 `.env` 中的明文 API Key，改用环境变量 | 安全风险 |

### P1（Sprint 2 核心交付）

| # | 行动项 | 原因 |
|---|--------|------|
| 5 | 实现"生成预算书"按钮逻辑（E4）| CartPage 空壳，核心流程断点 |
| 6 | 材料价格管理前端（E3）| 后端完整，前端缺失 |
| 7 | Excel 导入管理界面（E1）| 管理员无法使用，只能跑脚本 |
| 8 | 选择并统一一套前端（React 或 Streamlit）| 两套并存是技术债 |

### P2（代码质量）

| # | 行动项 | 原因 |
|---|--------|------|
| 9 | 删除废弃脚本（5个计量单位版 + regen_vectors.py）| 维护负担，容易混淆 |
| 10 | 统一数据库访问层（全部用 psycopg2 或全部 SQLAlchemy）| 混用增加 Bug 风险 |
| 11 | 删除 `routers/search.py` 和 `services/embedding.py` | 死代码，无调用方 |
| 12 | `backfill_cost_fields.py` 改为批量 DB 操作 | 3718 次往返，效率低 |
